...
def detect(original_image, original_lwir, detection, \
    ...
    original_lwir = Image.fromarray(original_lwir.astype(np.uint8)).convert('RGB')
    ...
    for i in range(det_boxes.shape[0]):
        ...
        draw.rectangle(xy=box_location, outline='#FF0000', width=width)  # Set color to yellow
        draw_lwir.rectangle(xy=box_location, outline='#FF0000', width=width)  # Set color to yellow
...
if __name__ == '__main__':
    ...
    visualize(arguments.result_dir, arguments.vis_dir, 'original')