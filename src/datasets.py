    ...
    def pull_item(self, index):
        ...
        if self.data == "KAIST":
            ...
            if self.mode == 'train':
                ...
                vis = Image.open( self._testimgpath % ( *frame_id[:-1], set_id, 'Visible', vid_id, img_id )).convert("RGB")
                lwir = Image.open( self._testimgpath % ( *frame_id[:-1], set_id, 'FIR', vid_id, img_id ) ).convert('L')
        elif self.data == "LLVIP":
            img_id = frame_id[-1]
            if self.mode == 'train':
        ...
        if self.mode == 'train': 
            ...
            if self.data == "KAIST":
                ...
                for line in open(self._annopath % ( *frame_id[:-1], "Annotations", img_id[0])) :
                    vis_boxes.append(line.strip().split())
                for line in open(self._annopath % ( *frame_id[:-1], "Annotations", img_id[0])) :
                    lwir_boxes.append(line.strip().split())
...
class KAISTPedWSEpoch(KAISTPed):
    ...
    def __getitem__(self, index):
        ...
        return vis, lwir, vis_box, lwir_box, vis_labels, lwir_labels, is_annotation
    ...
    def pull_item(self, index):
        ...
        if self.mode == 'train': 
            ...
            if id in self.teacher_image_ids.values() and self.aug_mode == "strong":
                ...
                vis_boxes = np.array(boxes, dtype=np.float64)
                lwir_boxes = np.array(boxes, dtype=np.float64)
...
class KAISTPedWSBatch(KAISTPedWSEpoch):
    ...
    def __getitem__(self, index):
        ...
        w_vis, w_lwir, s_vis, s_lwir, boxes, labels, is_annotation, img_paths = self.pull_item(index)
        ##print(self.ids[index])
        ##print(vis, lwir, boxes, labels, torch.ones(1,dtype=torch.int)*index, self.ids[index])
        #return w_vis, w_lwir, s_vis, s_lwir, boxes, labels, is_annotation, img_paths
    ...
    def pull_item(self, index):
        ...
        if self.data == "KAIST":
            set_id, vid_id, img_id = frame_id[-1]
            img_path = self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id)
            #print(img_path)
            vis = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'visible', img_id ))
            lwir = Image.open( self._imgpath % ( *frame_id[:-1], set_id, vid_id, 'lwir', img_id ) ).convert('L')
        elif self.data == "CVC-14":
            ...
            img_id = frame_id[-1]
            img_path = self._imgpath % ( *frame_id[:-1], 'visible', img_id[0] )
            if self.mode == 'train':
                vis = Image.open( self._imgpath % ( *frame_id[:-1], 'visible', img_id[0] )).convert("RGB")
                lwir = Image.open( self._imgpath % ( *frame_id[:-1], 'infrared', img_id[0] ) ).convert('L')
            else: 
       ...
       # print(*frame_id[:-1],"123" ,set_id, "123", vid_id, "123", 'visible', "123", img_id)
        ...
        if self.mode == 'train':
            ...
            if id in self.teacher_image_ids.values():
                ...
                return w_vis, w_lwir, s_vis, s_lwir, None, None, is_annotation, id
            else: 
                vis_boxes = list()
                lwir_boxes = list()
                if self.data == "KAIST":
        ...
        return vis, lwir, vis, lwir, boxes_t, labels, is_annotation, img_path
    ...
    def collate_fn(self, batch):
        ...
        return w_vis, w_lwir, s_vis, s_lwir, boxes, labels, is_anno #, img_paths
...
class LoadBox(object):
    ...
    def __call__(self, target, width, height):
        ...
        return np.array(res, dtype=np.float64)  # [[xmin, ymin, xmax, ymax, label_ind], ... ]
...
if __name__ == '__main__':
    ...
    xyxy_scaler_np = np.array([[input_w, input_h, input_w, input_h]], dtype=np.float64)
