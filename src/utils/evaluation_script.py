...
class KAISTPedEval(COCOeval):
    ...
    def accumulate(self, p=None):
        ...
        precisions = -np.ones((T, R, K, M))  # Initialize precision array
        ...
        for k, k0 in enumerate(setK):
            ...
            for m, maxDet in enumerate(setM):
                E = [self.evalImgs[Nk + i] for i in setI]
                ...
                for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                    ...
                    try:
                        for ri, pi in enumerate(inds):
                            q[ri] = recall[pi] if pi < len(recall) else 0
                            prec[ri] = precision[pi] if pi < len(precision) else 0
                    except Exception:
                        pass
                    ys[t, :, k, m] = np.array(q)
                    precisions[t, :, k, m] = np.array(prec)
    ...
    """
    def accumulate(self, p=None):
        def calculate_precision_recall_ap(self, ys, I0, npig, p, xx_graph):
            T, R, K, M = ys.shape
            precisions = -np.ones((T, R, K, M))  # precision array
            aps = np.zeros((T, K))  # AP per IoU threshold and category

            for t in range(T):
                for k in range(K):
                    for m in range(M):
                        q = ys[t, :, k, m]  # recall values at this threshold
                        p_list = []  # precision values
                        for r in range(R):
                            # precision = TP / (TP + FP)
                            # TP is recall * npig and FP is FPPI * I0
                            tp = q[r] * npig
                            fp = xx_graph[t][r] * I0
                            if tp + fp > 0:
                                precision = tp / (tp + fp)
                        ...
                        precisions[t, :, k, m] = p_list
                        
                        # prepare the recall and precision arrays for AP
                        recall_sorted = np.sort(q)  # sort by recall
                        precision_sorted = p_list[np.argsort(q)]  # reorder precision accordingly
                        
                        # maximum precision at each recall level
                        for i in range(precision_sorted.size - 2, -1, -1):
                            precision_sorted[i] = max(precision_sorted[i], precision_sorted[i + 1])
                        
                        # AP is the precision summed over the recall increments
                        recall_change = np.diff(recall_sorted)
                        aps[t, k] = np.sum(precision_sorted[:-1] * recall_change)
                        
            return precisions, aps
        ...
        precision, AP = calculate_precision_recall_ap(self, ys, I0, npig, p, xx_graph)
...
def evaluate(test_annotation_file: str, user_submission_file: str, phase_codename: str = 'Multispectral'):
    ...
    eval_result['near'] = copy.deepcopy(kaistEval)
    eval_result['medium'] = copy.deepcopy(kaistEval)
    eval_result['far'] = copy.deepcopy(kaistEval)

    # Near
    eval_result['near'].params.imgIds = imgIds
    eval_result['near'].params.HtRng = [[115, 1e5]]
    eval_result['near'].evaluate(0)
    eval_result['near'].accumulate()
    MR_near = eval_result['near'].summarize(0)

    # Medium
    eval_result['medium'].params.imgIds = imgIds
    eval_result['medium'].params.HtRng = [[45, 115]]
    eval_result['medium'].evaluate(0)
    eval_result['medium'].accumulate()
    MR_medium = eval_result['medium'].summarize(0)

    # Far
    eval_result['far'].params.imgIds = imgIds
    eval_result['far'].params.HtRng = [[1, 45]]
    eval_result['far'].evaluate(0)
    eval_result['far'].accumulate()
    MR_far = eval_result['far'].summarize(0)
    ...
    precision_all = eval_result['all'].eval['precision'][0][-1]
    AP_all = eval_result['all'].eval['AP'][0][0]

    #print(f"precision_all: {precision_all}")
    #print(f"AP_all: {AP_all}%")
    #print(f"precision_all: {precision_all}%")
    #print(f"precision_all: {precision_all * 100:.2f}%")
