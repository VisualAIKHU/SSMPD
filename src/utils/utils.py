...
def save_checkpoint(epoch, model, optimizer, loss, jobs_dir, name):
    ...
    state = {'epoch': epoch,
             ...
             'optimizer': optimizer}
    if name == "teacher":
        filename = os.path.join(jobs_dir,'teacher_checkpoint_ssd300.pth.tar{:03d}'.format(epoch))
    if name == "student":
        filename = os.path.join(jobs_dir,'student_checkpoint_ssd300.pth.tar{:03d}'.format(epoch))
